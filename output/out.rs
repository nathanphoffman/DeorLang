...
fn starts_binary_op(kind: String) -> bool {
    // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
    if kind == "PLUS" {
        // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
        return true;
...
fn first_depth0_newline(tokens: TokensRef, start: i64, end_pos: i64) -> i64 {
    // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
    let mut depth: i64 = 0;
    let mut scan_pos: i64 = start.clone();
...
fn validate_tokens(tokens: TokensRef) {
    ...
    let mut lbl_stmt: String = "statement".to_string();
    ...
    let mut rule_missing_return_expr: String = "looks like a value meant to be returned, but 'return' is missing — only a function body that's a single expression implicitly returns; add 'return' here".to_string();
    ...
    while pos < token_count {
        ...
        // macro: check_fn_decl (transpiler-deor/tokens_validator/macros/declarations/check_fn_decl.deor)
        {
            ...
            // macro: check_missing_return_expr (transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor)
            {
                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                fn locate_fn_return_type(kw_pos: i64) -> i64 {
                    return kw_pos + 1;
                ...
                if cur_kind == "KW_FN" {
                    ...
                    if left_paren_pos < token_count {
                        ...
                        let mut left_paren_token: Token = tokens[left_paren_pos as usize].clone();
                        let mut kind = left_paren_token.kind.clone();
                        let mut is_call_shape: bool = kind == "LPAREN";
                        if has_ret_type && !is_void && is_call_shape {
                            // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                            let mut right_paren_pos: i64 = find_matching_rparen(tokens.clone(), left_paren_pos.clone());
                            let mut newline_pos: i64 = right_paren_pos + 1;
                            if newline_pos < token_count {
                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                let mut newline_token: Token = tokens[newline_pos as usize].clone();
                                ...
                                if kind == "NEWLINE" {
                                    ...
                                    if indent_pos < token_count {
                                        // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                        let mut indent_token: Token = tokens[indent_pos as usize].clone();
                                        let mut kind = indent_token.kind.clone();
                                        if kind == "INDENT" {
                                            // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                            let mut body_first_pos: i64 = indent_pos + 1;
                                            let mut body_end_pos: i64 = find_block_end_ref(tokens.clone(), indent_pos.clone());
                                            if body_first_pos < body_end_pos {
                                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                let mut first_nl_pos: i64 = first_depth0_newline(tokens.clone(), body_first_pos.clone(), body_end_pos.clone());
                                                let mut is_single_stmt: bool = false;
                                                let mut found_nl: bool = first_nl_pos < body_end_pos;
                                                if found_nl {
                                                    // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                    let mut after_first_nl: i64 = first_nl_pos + 1;
                                                    is_single_stmt = after_first_nl == body_end_pos;
                                                }
                                                if !is_single_stmt {
                                                    // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                    let mut scan_pos: i64 = body_first_pos.clone();
                                                    while scan_pos < body_end_pos {
                                                        // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                        let mut at_boundary: bool = scan_pos == body_first_pos;
                                                        if !at_boundary {
                                                            // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                            let mut prev_pos: i64 = scan_pos - 1;
                                                            let mut prev_token: Token = tokens[prev_pos as usize].clone();
                                                            let mut kind = prev_token.kind.clone();
                                                            let mut prev_kind: String = kind.clone();
                                                            let mut prev_is_boundary: bool = prev_kind == "NEWLINE";
                                                            if !prev_is_boundary {
                                                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                prev_is_boundary = prev_kind == "INDENT";
                                                            }
                                                            if !prev_is_boundary {
                                                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                prev_is_boundary = prev_kind == "DEDENT";
                                                            }
                                                            at_boundary = prev_is_boundary;
                                                        }
                                                        if at_boundary {
                                                            // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                            let mut stmt_token: Token = tokens[scan_pos as usize].clone();
                                                            let mut kind = stmt_token.kind.clone();
                                                            let mut stmt_kind: String = kind.clone();
                                                            let mut is_candidate: bool = stmt_kind == "IDENT";
                                                            if !is_candidate {
                                                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                is_candidate = stmt_kind == "INT";
                                                            }
                                                            if !is_candidate {
                                                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                is_candidate = stmt_kind == "FLOAT";
                                                            }
                                                            if !is_candidate {
                                                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                is_candidate = stmt_kind == "STRING";
                                                            }
                                                            if !is_candidate {
                                                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                is_candidate = stmt_kind == "KW_TRUE";
                                                            }
                                                            if !is_candidate {
                                                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                is_candidate = stmt_kind == "KW_FALSE";
                                                            }
                                                            if is_candidate {
                                                                // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                let mut next_pos: i64 = scan_pos + 1;
                                                                if next_pos < body_end_pos {
                                                                    // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                    let mut next_token: Token = tokens[next_pos as usize].clone();
                                                                    let mut kind = next_token.kind.clone();
                                                                    let mut is_bare_expr: bool = starts_binary_op(kind.clone());
                                                                    if !is_bare_expr {
                                                                        // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                        is_bare_expr = kind == "NEWLINE";
                                                                    }
                                                                    if is_bare_expr {
                                                                        // transpiler-deor/tokens_validator/macros/declarations/check_missing_return_expr.deor
                                                                        errors.push(val_err(stmt_token.clone(), lbl_stmt.clone(), rule_missing_return_expr.clone()).clone());
                                                                    }
                                                                }
                                                            }
                                                        }
    ...
    if kind == "LPAREN" {
        // macro: tb_paren (transpiler-deor/codegen/decl/stmt/macros/typed_binding/tb_paren.deor)
        {
            ...
            if is_avow_expr {
                ...
                let mut left_code: String = unwrap_expr.clone();
                let mut cur_pos: i64 = after_rparen.clone();
                while cur_pos < token_count {
                    // transpiler-deor/codegen/decl/stmt/macros/typed_binding/tb_paren.deor
                    let mut operator_token: Token = tokens[cur_pos as usize].clone();
                    let kind = operator_token.kind.clone();
                ...
                unwrap_expr = left_code;
                after_rparen = cur_pos;
...
fn gen_fn_decl(fn_tokens: TokensRef, pos: i64, ctx: RcCtx) -> ParseResult {
    ...
    let mut detect_tokens: TokensRef = tokens.clone();
    ...
    let mut used_implicit_return: bool = false;
    let mut implicit_return_code: String = "".to_string();
    let mut implicit_return_end: i64 = 0;
    let mut is_void_ret: bool = is_empty(ret_type.clone());
    if !is_void_ret {
        // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
        if body_len > 2 {
            // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
            let mut last_idx: i64 = body_len - 1;
            let mut second_last_idx: i64 = body_len - 2;
            let mut last_tok: Token = detect_tokens[last_idx as usize].clone();
            let kind = last_tok.kind.clone();
            let mut ends_in_dedent: bool = kind == "DEDENT";
            let mut second_last_tok: Token = detect_tokens[second_last_idx as usize].clone();
            let kind = second_last_tok.kind.clone();
            let mut second_last_is_newline: bool = kind == "NEWLINE";
            if ends_in_dedent && second_last_is_newline {
                // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                let mut first_tok: Token = detect_tokens[zero as usize].clone();
                let kind = first_tok.kind.clone();
                let mut first_kind: String = kind.clone();
                let mut skip_leading: bool = false;
                if first_kind == "KW_RETURN" {
                    // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                    skip_leading = true;
                }
                if first_kind == "KW_IF" {
                ...
                if first_kind == "KW_FOR" {
                    // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                    skip_leading = true;
                }
                if first_kind == "KW_CONST" {
                    // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                    skip_leading = true;
                }
                if first_kind == "KW_RAW" {
                    // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                    skip_leading = true;
                }
                if first_kind == "KW_RUST" {
                    // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                    skip_leading = true;
                }
                if first_kind == "KW_BLOCK" {
                    // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                    skip_leading = true;
                }
                if first_kind == "KW_MACRO_RUN" {
                    // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                    skip_leading = true;
                }
                if first_kind == "KW_UNSAFE_MACRO_RUN" {
                    // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                    skip_leading = true;
                }
                if first_kind == "LPAREN" {
                    // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                    skip_leading = true;
                }
                if !skip_leading {
                    ...
                    if fully_consumed {
                        // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                        let mut is_chain: bool = is_expr_chain(detect_tokens.clone(), zero.clone(), trial_end.clone());
                        let mut suffix: String = "".to_string();
                        if !is_chain {
                            // transpiler-deor/codegen/decl/macros/fn_build_body_ctx.deor
                        ...
                        let mut return_kw: String = "return ".to_string();
                        let mut return_sc: String = ";\n".to_string();
                        let mut return_code: String = [RS_IND.as_str(), return_kw.as_str(), trial_code.as_str(), suffix.as_str(), return_sc.as_str()].concat();
    ...
    // macro: fn_emit (transpiler-deor/codegen/decl/macros/fn_emit.deor)
    {
        ...
        if used_implicit_return {
            // transpiler-deor/codegen/decl/macros/fn_emit.deor
            let mut fn_code: String = [fnc_kw.as_str(), fn_name.as_str(), RS_LP.as_str(), params_code.as_str(), RS_RP.as_str(), ret_suffix.as_str(), RS_OB.as_str(), implicit_return_code.as_str(), RS_FNC_CB.as_str()].concat();
            let mut implicit_body_end: i64 = body_start + implicit_return_end;
